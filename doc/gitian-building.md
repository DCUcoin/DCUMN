...
*Setup instructions for a Gitian build of DCU Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the DCU
...
- [Building DCU Core](#building-dcu-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-dcu-host/README.md).
...
Clone the git repositories for DCU Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/dcucrypto/dcu
```
...
in the DCU Core repository under 'Fetch and build inputs' to install sources which require
...
To build DCU Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the DCU Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/dcu/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/dcucrypto/dcu
...
For example:
```bash
URL=https://github.com/crowning-/dcu.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit dcu=${COMMIT} --url dcu=${URL} ../dcu/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit dcu=${COMMIT} --url dcu=${URL} ../dcu/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit dcu=${COMMIT} --url dcu=${URL} ../dcu/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the dcu git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../dcu/contrib/gitian-descriptors/*|sort|uniq )
...
DCUPATH=/some/root/path/dcu.git
SIGPATH=/some/root/path/dcu-detached-sigs.git

./bin/gbuild --url dcu=${DCUPATH},signature=${SIGPATH} ../dcu/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/dcu-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/dcu-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/dcu-osx-build.assert
```
...
[dcu/gitian.sigs](https://github.com/dcucrypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
