...
const char * const DCU_CONF_FILENAME = "dcu.conf";
const char * const DCU_PID_FILENAME = "dcud.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "dcu" is a composite category enabling all DCU-related debug output
            if(ptrCategory->count(string("dcu"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DCUCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DCUCore
    // Mac: ~/Library/Application Support/DCUCore
    // Unix: ~/.dcucore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DCUCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DCUCore";
#else
    // Unix
    return pathRet / ".dcucore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", DCU_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty dcu.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override dcu.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", DCU_PID_FILENAME));
