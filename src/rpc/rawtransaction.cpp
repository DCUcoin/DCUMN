        ...
        if (tx.IsCoinBase())
            ...
            if (GetSpentIndex(spentKey, spentInfo)) {
                ...
                if (spentInfo.addressType == 1) {
                    in.push_back(Pair("address", CDCUAddress(CKeyID(spentInfo.addressHash)).ToString()));
                } else if (spentInfo.addressType == 2)  {
                    in.push_back(Pair("address", CDCUAddress(CScriptID(spentInfo.addressHash)).ToString()));
                }
            }
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric or string, required) The key is the dcu address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"MbWMQqUNEosjjEb9WAGuJ5KGN9h4WL5bqf\"   (string) DCU address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue decodescript(const UniValue& params, bool fHelp)
{
    ...
    if (type.isStr() && type.get_str() != "scripthash") {
        ...
        r.push_back(Pair("p2sh", CDCUAddress(CScriptID(script)).ToString()));
    }
