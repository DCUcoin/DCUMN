...
UniValue masternode(const UniValue& params, bool fHelp)
{
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        ...
        obj.push_back(Pair("outpoint",      mnInfo.vin.prevout.ToStringShort()));
        obj.push_back(Pair("payee",         CDCUAddress(mnInfo.pubKeyCollateralAddress.GetID()).ToString()));
    ...
    if (strCommand == "genkey")
    {
        ...
        return CDCUSecret(secret).ToString();
    }
    ...
    if (strCommand == "status")
    {
        ...
        if(mnodeman.Get(activeMasternode.outpoint, mn)) {
            mnObj.push_back(Pair("payee", CDCUAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }
...
UniValue masternodelist(const UniValue& params, bool fHelp)
{
                ...
                strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print DCU address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
        ...
        for (auto& mnpair : mapMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               CDCUAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " <<
                ...
                CDCUAddress address(mn.pubKeyCollateralAddress.GetID());
    ...
    if (strCommand == "decode")
    {
        ...
        BOOST_FOREACH(CMasternodeBroadcast& mnb, vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", CDCUAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
                resultObj.push_back(Pair("pubKeyMasternode", CDCUAddress(mnb.pubKeyMasternode.GetID()).ToString()));
