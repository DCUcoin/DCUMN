...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or " + std::string(DCU_CONF_FILENAME) + " setting gen)\n"
...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
    ...
    GenerateDCUs(fGenerate, nGenProcLimit, Params(), *g_connman);
...
// NOTE: Unlike wallet RPC (which use DCU values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    https://github.com/dcu/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/dcu/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "DCU Core is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "DCU Core is downloading blocks...");
    ...
    if (sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)
        ...
        && !mnpayments.GetBlockPayee(chainActive.Height() + 1, payee))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "DCU Core is downloading masternode winners...");

    // next bock is a superblock and we need governance info to correctly construct it
    if (sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED)
        && !masternodeSync.IsSynced()
        && CSuperblock::IsValidBlockHeight(chainActive.Height() + 1))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "DCU Core is syncing with network...");
...
UniValue submitblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "See https://en.dcu.it/wiki/BIP_0022 for full specification.\n"
...
UniValue estimatesmartfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric) estimate fee-per-kilobyte (in DCU)\n"
