             ...
             CWalletScanState &wss, string& strType, string& strErr)
{
    try {
        ...
        if (strType == "name")
        {
            string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CDCUAddress(strAddress).Get()].name;
        }
        else if (strType == "purpose")
        {
            string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CDCUAddress(strAddress).Get()].purpose;
        }
        else if (strType == "tx")
        {
            uint256 hash;
        ...
        else if (strType == "destdata")
        {
            ...
            ssKey >> strKey;
            ssValue >> strValue;
            if (!pwallet->LoadDestData(CDCUAddress(strAddress).Get(), strKey, strValue))
            {
