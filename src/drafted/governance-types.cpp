...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "DCUNetwork") type = DCUNetwork;
    if(strType == "DCUNetworkVariable") type = DCUNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == DCUNetwork) s = "DCUNetwork";
    if(type == DCUNetworkVariable) s = "DCUNetworkVariable";
