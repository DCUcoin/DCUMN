...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // NOTE: unlike in dcu, we need to pass PREVIOUS block height here
...
// ***TODO*** ScanHash is not yet used in DCU
//
...
// ***TODO*** that part changed in dcu, we are using a mix with old one here for now
void static DCUMiner(const CChainParams& chainparams, CConnman& connman)
{
    LogPrintf("DCUMiner -- started\n");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("DCUMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("DCUMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("DCUMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("DCUMiner -- runtime error: %s\n", e.what());
        return;
    }
}

void GenerateDCUs(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DCUMiner, boost::cref(chainparams), boost::ref(connman)));
}
