...
const std::string DCUGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString DCUGUI::DEFAULT_WALLET = "~Default";

DCUGUI::DCUGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QPixmap pixmapLogo(":/images/light/dcu_logo_horizontal");
...
void DCUGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a DCU address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and dcu: URIs)"));
    ...
    aboutAction = new QAction(tr("&About DCU"), this);
    aboutAction->setStatusTip(tr("Show information about DCU"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for DCU"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your DCU addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified DCU addresses"));
    ...
    openAction->setStatusTip(tr("Open a dcu: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the DCU help message to get a list with possible DCU command-line options"));
...
void DCUGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void DCUGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool DCUGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool DCUGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void DCUGUI::setWalletActionsEnabled(bool enabled)
{
...
void DCUGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("DCU client") + " " + networkStyle->getTitleAddText();
...
void DCUGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void DCUGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void DCUGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void DCUGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void DCUGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void DCUGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void DCUGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void DCUGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void DCUGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void DCUGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void DCUGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void DCUGUI::updateNetworkState()
{
...
void DCUGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void DCUGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void DCUGUI::updateHeadersSyncProgressLabel()
{
...
void DCUGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void DCUGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void DCUGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("DCU"); // default title
...
void DCUGUI::changeEvent(QEvent *e)
{
...
void DCUGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void DCUGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void DCUGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(DCUUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void DCUGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void DCUGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool DCUGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool DCUGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void DCUGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void DCUGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void DCUGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void DCUGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void DCUGUI::detectShutdown()
{
...
void DCUGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void DCUGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void DCUGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(DCUGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void DCUGUI::subscribeToCoreSignals()
{
...
void DCUGUI::unsubscribeFromCoreSignals()
{
...
void DCUGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void DCUGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<DCUUnits::Unit> units = DCUUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const DCUUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(DCUUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(DCUUnits::Unit u, DCUUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(DCUUnits::name(u)), this);
...
{
    setText(DCUUnits::name(newUnits));
}
