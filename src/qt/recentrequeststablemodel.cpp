    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return DCUUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, DCUUnits::separatorNever);
            else
                return DCUUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
QString RecentRequestsTableModel::getAmountTitle()
{
    ...
    {
        amountTitle += " ("+DCUUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")";
    }
    return amountTitle;
}
