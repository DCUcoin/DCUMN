    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("DCU Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About DCU Core"));
        ...
        std::string strUsage = HelpMessage(HMM_DCU_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", DCUGUI::DEFAULT_UIPLATFORM));
        }
...
All the DCU in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 DCU, 0.1 DCU, 1 DCU and 10 DCU -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("DCU Core is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

QWidget *ShutdownWindow::showShutdownWindow(DCUGUI *window)
{
    if (!window)
        return nullptr;
