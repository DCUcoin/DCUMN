...
###   This script attempts to download the signature file SHA256SUMS.asc from dcu.org
...
RCSUBDIR="test/"
BASEDIR="https://dcu.org/bin/"
VERSIONPREFIX="dcu-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'dcu-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the DCU Core binary release signing key installed?"
   fi
